...
# ===========================================================================
HOST = "sanjanaaa79"   # shown as  you@github  in the header
...
ROWS = [
    ...
    ("kv", "Role", "Student"),
    ("kv", "University", "SRM Institute of Science and Technology"),
    ("kv", "Languages", "Python • C++ • JavaScript"),
    ("kv", "Frontend", "React Native • Vite"),
    ("kv", "Backend", "Node.js • MySQL"),
    ("kv", "AI/ML", "TensorFlow • PyTorch • OpenCV"),
    ("kv", "Cyber", "PortSwigger • TryHackMe"),
    ("kv", "Interests", "AI • Cybersecurity • Full Stack"),

    ("gap",),

    ("sec", "Highlights"),
    ("bul", "Won third prize in a national-level hackathon"),
]
...
for i, row in enumerate(ROWS):
    ...
    if kind == "gap":
        ...
        inner = (
            ...
            f'fill="{SECTION}" font-size="12.5" font-weight="700">'
            f'&#8212; {title}</text>'
        ...
        key = html.escape(str(row[1]), quote=False)
        val = html.escape(str(row[2]), quote=False)

        # restore HTML entities such as &#8226;
        val = val.replace("&amp;#", "&#")

        inner = (
            f'<text x="{KEY_X}" y="{y:.1f}" '
            f'fill="{KEY}" font-size="12.5" font-weight="700">{key}</text>'
            f'<text x="{VAL_X}" y="{y:.1f}" '
            f'fill="{INK}" font-size="12.5">{val}</text>'
        )

    elif kind == "bul":
        txt = html.escape(str(row[1]), quote=False)

        inner = (
            f'<circle cx="{KEY_X+3}" cy="{y-4:.1f}" '
            f'r="2.5" fill="{GREEN}"/>'
            f'<text x="{KEY_X+14}" y="{y:.1f}" '
            f'fill="{INK}" font-size="12.5">{txt}</text>'
        )
